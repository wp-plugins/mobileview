 ...
 *
 *		This method should be called inside the HEAD area of a WPMobi theme template file.
 *		This method ultimately intercepts the main WordPress wp_head action.
 *
 *		The WordPress action \em wpmobi_pre_head is executed at the start of the header block, and the action \em wpmobi_post_head is executed at the end
 *		of the header block.
 *
 *		\par Typical Usage:
 *		\include wpmobi-head.php
 *
 *		\ingroup templatetags
 */	 
function wpmobi_head() {
	do_action( 'wpmobi_pre_head' );	
	
	wp_head();		
	
	do_action( 'wpmobi_post_head' );
}

/*!		\brief Called inside a template file prior to the closing BODY tag.
 *
 *		This method should be called just prior to the closing BODY tag inside of a WPMobi theme template file.
 *		This method ultimately intercepts the main WordPress \em wp_footer action.  
 *
 *		The WordPress action \em wpmobi_pre_footer is executed at the start of the footer block, and the action \em wpmobi_post_footer is executed at the end
 *		of the footer block.
 *
 *		\par Typical Usage:
 *		The following is the typical usage of this method and proper placement in the HTML structure:
 *
 *		\include wpmobi-footer.php
 *
 *		\ingroup templatetags 
 */	 
function wpmobi_footer() {
	do_action( 'wpmobi_pre_footer' );
	
	wp_footer();	
	
	do_action( 'wpmobi_post_footer' );
}
 ...
 *		\include wpmobi-title.php
 *
 *		\ingroup templatetags 
 */
function wpmobi_title() {
	if ( is_home() ) {
		echo wpmobi_bloginfo( 'site_title' );
	} else {
		echo wpmobi_bloginfo( 'site_title' ) . wp_title( ' &raquo; ', 0 );	
	}
}

/*!		\brief Echos the short title for the WordPress site
 *
 *		This method echos the result of the wpmobi_get_site_title() method.
 *
 *		\ingroup templatetags 
 */
function wpmobi_site_title() {
	echo wpmobi_get_site_title();
}
 ...
 *
 *		This method can be filtered using the WordPress filter \em wpmobi_site_title.
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_site_title() {
	global $wpmobi;	
	$settings = $wpmobi->get_settings();
	
	return apply_filters( 'wpmobi_site_title', $settings->site_title );
}
 ...
 *
 *		\par Typical Usage:
 *		The following is an example of a WPMobi theme loop:
 *		\include theme-loop.php
 *
 *		\ingroup templatetags 
 */
function wpmobi_have_posts() {
	return have_posts();
}

/*!		\brief Populates the post data in a WPMobi theme loop.
 *
 *		This method will populate the post data structure and is meant to be used in conjunction with wpmobi_have_posts().
 *
 *		\par Typical Usage:
 *		The following is an example of a WPMobi theme loop:
 *		\include theme-loop.php 
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_post() {
	the_post();
}

/*!		\brief Echos the post content returned from wpmobi_get_content()
 *
 *		This method echos the post content returned from wpmobi_get_content.  This is a wrapper for the WordPress
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_content() {
	echo apply_filters( 'the_content', wpmobi_get_content() );
}

/*!		\brief Return the post content associated with a post.
 *
 *		This is a wrapper for the WordPress method \em get_the_content.
 *		This method can be filtered by using the WordPress filter \em wpmobi_the_content.
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_content() {
	return apply_filters( 'wpmobi_the_content', get_the_content() );
}

/*!		\brief Echos the post excerpt associated with a post.
 *
 *		This method echos the output from wpmobi_get_excerpt().
 *
 *		\ingroup templatetags 
 */
function wpmobi_the_excerpt() {
	echo wpmobi_get_excerpt();	
}
 ...
 *		WordPress filter \em wpmobi_excerpt.
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_excerpt() {
	return apply_filters( 'wpmobi_excerpt', get_the_excerpt() );
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_footer_classes() {
	echo wpmobi_get_footer_classes();
}
 ...
 *		This method can be filtered using the WordPress filter \em wpmobi_footer_classes.
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_footer_classes() {
	$footer_classes = array( 'footer' );
	
	return implode( ' ', apply_filters( 'wpmobi_footer_classes', $footer_classes ) );
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_body_classes() {
	echo wpmobi_get_body_classes();
}

/*!		\brief Returns a space-separated list of classes to be used for the main body tag.
 *
 *		This method echos a list of the recommended classes to use for the main body tag in each theme.
 *		This method can be filtered using the WordPress filter \em wpmobi_body_classes.
 *
 *		\par Added Classes
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_body_classes() {
	global $wpmobi;
	$settings = $wpmobi->get_settings();
	
	$body_classes = array( 'wpmobi-me' );
	$mobile_device = $wpmobi->get_active_mobile_device();
	if ( $mobile_device ) {
		$body_classes[] = 'device-' . wpmobi_make_css_friendly( $mobile_device );	
	}
	
	$active_device_class = $wpmobi->get_active_device_class();
	if ( $active_device_class ) {
		$body_classes[] = 'device-class-' . wpmobi_make_css_friendly( $active_device_class );	
	}
	
	$current_skin = $wpmobi->get_current_theme_skin();
	if ( $current_skin ) {
		$body_classes[] = 'skin-' . wpmobi_make_css_friendly( basename( $current_skin, '.css' ) );
	}	
	
	$body_classes[] = 'theme-' . wpmobi_make_css_friendly( $wpmobi->get_current_theme() );	
	
	if ( is_page() || wpmobi_is_custom_page_template() ) {
		global $post;
		
		if ( wpmobi_is_custom_page_template() ) {
			$page_id = wpmobi_get_custom_page_template_id();
		} else {
			$page_id = $post->ID;
		}
		...
		if ( isset( $settings->menu_icons[ $page_id ] ) ) {
			...
			if ( $current_icon ) {
				...
				$icon_set = $wpmobi->get_set_with_icon( $current_icon );
				if ( $icon_set ) {
	...
	return implode( ' ', apply_filters( 'wpmobi_body_classes', $body_classes ) );	
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_make_css_friendly( $name ) {
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_title() {
	echo wpmobi_get_title();	
}
 ...
 * 		This method can be filtered using the WordPress filter \em wpmobi_the_title.
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_title() {
	return apply_filters( 'wpmobi_the_title', get_the_title() );	
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_permalink() {
	echo wpmobi_get_the_permalink(); 
}
 ...
 *		This method can be filtered using the WordPress \em wpmobi_the_permalink filter.
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_the_permalink() {
	return apply_filters( 'wpmobi_the_permalink', get_permalink() );	
}
 ...
 *		wpmobi_get_post_classes().
 *
 *		\ingroup templatetags 
 */
function wpmobi_post_classes() {
	echo implode( ' ', wpmobi_get_post_classes() );	
}
 ...
 *		with the WordPress filter \em wpmobi_post_classes.
 *
 *		\par Added Classes:
 *		The following classes are added automatically:
 ...
 *		\ingroup templatetags
 */
function wpmobi_get_post_classes() {
	global $post;
	...
	if ( wpmobi_has_post_thumbnail() ) {
		$post_classes[] = 'has-thumbnail';
	} else {
		$post_classes[] = 'no-thumbnail';
	}

	return apply_filters( 'wpmobi_post_classes', $post_classes );
}
 ...
 *		\ingroup templatetags
 */
function wpmobi_has_post_thumbnail() {
	if ( function_exists( 'has_post_thumbnail' ) ) {
		$has_thumbnail = has_post_thumbnail();
		
		return apply_filters( 'wpmobi_has_post_thumbnail', $has_thumbnail );
	} else {
		return apply_filters( 'wpmobi_has_post_thumbnail', false );
	}
}
...
function wpmobi_get_the_post_thumbnail( $param = false ) {
	...
	if ( function_exists( 'has_post_thumbnail' ) && has_post_thumbnail() ) {
		...
		return apply_filters( 'wpmobi_the_post_thumbnail', $thumbnail, $param );
}
 ...
 *		wpmobi_get_content_classes().
 *
 *		\ingroup templatetags
 */
function wpmobi_content_classes() {
	echo implode( ' ', wpmobi_get_content_classes() );
}
 ...
 *		This method can be filtered using the WordPress filter \em wpmobi_content_classes.
 *
 *		\note Currently only the class 'content' is added, but additional classes will be added in future versions
 *
 *		\ingroup templatetags
 */
function wpmobi_get_content_classes() {
	$content_classes = array( 'content' );

	return apply_filters( 'wpmobi_content_classes', $content_classes );
}
 ...
 *		wpmobi_get_date_classes().
 *
 *		\ingroup templatetags
 */
function wpmobi_date_classes() {
	echo implode( ' ', wpmobi_get_date_classes() );
}
 ...
 *		This method can be filtered using the WordPress filter \em wpmobi_date_classes.
 *
 *		\par Added Classes:
 *		The following classes are added automatically:
 ...
 *
 *		\par Typical Usage:
 *		The following shows a typical content loop and usage of wpmobi_date_classes():
 *		\include theme-loop.php
 *
 *		\ingroup templatetags
 */
function wpmobi_get_date_classes() {
	$date_classes = array();
	$date_classes[] = 'date';
	...
	return apply_filters( 'wpmobi_date_classes', $date_classes );
}

/*!		\brief Echos the date for a post.
 *
 *		This function echos the result of wpmobi_get_the_time().
 *
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_time( $format ) {
	echo wpmobi_get_the_time( $format );	
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_the_time( $format ) {
	$settings = wpmobi_get_settings();
	if ( $settings->respect_wordpress_date_format ) {
		$format = get_option('date_format');	
	}
	
	return apply_filters( 'wpmobi_get_the_time', get_the_time( $format ) );
}
 ...
 *		\ingroup templatetags
 */
function wpmobi_has_tags() {
	if ( is_page() ) {
		return false;	
	}
	
	$tags = get_the_tags();
	return apply_filters( 'wpmobi_has_tags', $tags );
}
 ...
 *		\ingroup templatetags
 */
function wpmobi_has_categories() {
	if ( is_page() ) {
		return false;
	}
 ...
 *		wpmobi_has_categories().
 *
 *		\ingroup templatetags 
 */
function wpmobi_the_categories() {
	the_category( ', ' );
}
 ...
 *		\ingroup templatetags
 */
function wpmobi_has_next_posts_link() {
	ob_start();
	next_posts_link();
...
/*!		\brief Executes a non-standard template file in the current WPMobi theme directory.
 *
 *		This function executes a non-standard template file in the current WPMobi theme directory.  The standard template files 
 *		(index.php, header.php, etc) for WordPress will work with WPMobi.  Non-standard template files can be
 *		included by using this function.  For example, wpmobi_do_template( 'gliding-menu.php' ) will execute the gliding-menu.php
 *		PHP script that is located in the current WPMobi theme directory.  
 *
 ...
 *		\ingroup templatetags 
 */
function wpmobi_do_template( $template_name ) {
	global $wpmobi;
	$template_path = $wpmobi->get_current_theme_directory() . '/' . $wpmobi->get_active_device_class() . '/' . $template_name;
	$directories = array( TEMPLATEPATH );
	if ( $wpmobi->is_child_theme() ) { 
 ...
 *		\ingroup templatetags 
 */
function wpmobi_is_custom_page_template() {
	global $wpmobi;
	return $wpmobi->is_custom_page_template;
}


/*!		\brief Used to echo the current custom page template ID
 *
 *		This function will echo the results from wpmobi_get_custom_page_template_id(). 
 *
 *		\ingroup templatetags 
 */
function wpmobi_the_custom_page_template_id() {
	echo wpmobi_get_custom_page_template_id();
}

/*!		\brief Used to determine the current custom page template ID
 *
 *		This function can be used to determine the current custom page template ID.  This method should be used in conjunction with wpmobi_is_custom_page_template().
 *
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_custom_page_template_id() {
	global $wpmobi;	
	return $wpmobi->custom_page_template_id;
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_page_get_icon() {
	global $wpmobi;
	
	if ( wpmobi_page_has_icon() ) {
		if ( wpmobi_is_custom_page_template() ) {
			$page_id = wpmobi_get_custom_page_template_id();
		} else {
			$page_id = get_the_ID();
		...
		if ( $settings->custom_menu_name != 'none' ) {
			$menu_items = array();
			wpmobi_build_wordpress_menu_tree( 0, 1, $menu_items, true );

			$icon = wpmobi_recursive_find_page( $page_id, $menu_items );
			if ( $icon ) {
				return $icon;	
			} else {
				return wpmobi_get_site_menu_icon( -1000000 );
			}
		} else {
			return wpmobi_get_site_menu_icon( $page_id );
		}
	} else {
		return false;	
	}	
...
function wpmobi_recursive_find_page( $page_id, $menu_items ) {
	$icon = false;
	
	foreach( $menu_items as $key => $item ) {
		if ( isset( $item->original_id ) && $item->original_id == $page_id ) {
			$icon = wpmobi_get_site_menu_icon( $item->page_id );
		} else if ( isset( $item->has_children ) && $item->has_children ) {
			$icon = wpmobi_recursive_find_page( $page_id, $item->submenu );
		} 
 ...
 *		\ingroup templatetags 
 */
function wpmobi_page_the_icon() {
	$icon = wpmobi_page_get_icon();
	if ( $icon ) {
		echo $icon;	
	}
}
 ...
 *
 *		This function echos the mobile/desktop switch link URL.  It echos the result from wpmobi_get_mobile_switch_link().
 *
 *		\ingroup templatetags 
 */
function wpmobi_the_mobile_switch_link() {
	echo wpmobi_get_mobile_switch_link();
}

/*!		\brief Retrieves the mobile/desktop switch link URL for mobile theme
 *
 *		This function can be used to retrieve the mobile/desktop switch link URL.  It can be filtered using the WordPress filter \em wpmobi_mobile_switch_link.
 *
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_mobile_switch_link() {
	$settings = wpmobi_get_settings();
	
	if ( $settings->show_switch_link ) {
		if ( $settings->home_page_redirect_address == 'same' ) {
			return apply_filters( 'wpmobi_mobile_switch_link', get_bloginfo( 'url' ) . '?wpmobi_switch=desktop&amp;redirect=' . urlencode( $_SERVER['REQUEST_URI'] ) );
		} else {
			return apply_filters( 'wpmobi_mobile_switch_link', get_bloginfo( 'url' ) . '?wpmobi_switch=desktop&amp;redirect=' . get_bloginfo( 'url' ) );
		}
	}
}
 ...
 *		administration panel for WPMobi, and is stored in the setting \em welcome_alert.
 *
 *		\returns True if a welcome message should be shown, false otherwise
 *
 *		\ingroup templatetags 
 */
function wpmobi_has_welcome_message() {
	$settings = wpmobi_get_settings();
	if ( isset( $_COOKIE['wpmobi_welcome'] ) && $_COOKIE['wpmobi_welcome'] === '1' ) {
 ...
 *
 *		This function can be used to echo the welcome message retrieved using wpmobi_get_welcome_message().
 *
 *		\note The welcome message is currently disabled work in Web-App Mode
 *
 *		\ingroup templatetags 
 */
function wpmobi_the_welcome_message() {
	echo wpmobi_get_welcome_message();
}
 ...
 *		be filtered by the WordPress filter \em wpmobi_welcome_message. 
 *
 *		\note The welcome message is currently disabled work in Web-App Mode
 *
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_welcome_message() {
	$settings = wpmobi_get_settings();
	return apply_filters( 'wpmobi_welcome_message', $settings->welcome_alert );
}

/*!		\brief Echos the 404 message
 *
 *		This function can be used to echo the 404 message retrieved using wpmobi_get_404_message().
 *
 *		\note The 404 message is currently only shown in English
 *
 *		\ingroup templatetags 
 */
function wpmobi_the_404_message() {
	echo wpmobi_get_404_message();
}
 ...
 *		be filtered by the WordPress filter \em wpmobi_404_message. 
 *
 *		\returns A string representing the 404 message
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_404_message() {
	$settings = wpmobi_get_settings();
	return apply_filters( 'wpmobi_404_message', $settings->fourohfour_message );
}

/*!		\brief Echos the footer message
 *
 *		This function can be used to echo the footer message retrieved using wpmobi_get_the_footer_message(). It also wraps paragraph tags
 *		around the footer message.
 * 
 *		\ingroup templatetags 
 */
function wpmobi_the_footer_message() {
	echo '<p>';
	echo wpmobi_get_the_footer_message();
	echo '</p>';
}
 ...
 *		of this function can be filtered using the WordPress filter \em wpmobi_footer_message.
 * 
 *		\returns A string representing the footer message
 *
 *		\ingroup templatetags 
 */
function wpmobi_get_the_footer_message() {
	$settings = wpmobi_get_settings();
	return apply_filters( 'wpmobi_footer_message', $settings->footer_message );
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_welcome_message_dismiss_url() {
	echo wpmobi_get_welcome_message_dismiss_url();
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_welcome_message_dismiss_url() {
	return apply_filters( 'wpmobi_welcome_message_dismiss_url', get_bloginfo( 'url' ) . '?wpmobi=dismiss_welcome&amp;redirect=' . $_SERVER['REQUEST_URI'] );
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_comment_count() {
	echo wpmobi_get_comment_count();	
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_comment_count() {
	global $wpdb;
	global $post;
 ...
 *		\ingroup templatetags 
 */
function wpmobi_ordered_cat_list( $num ) {
	global $wpdb;
 ...
 *		\ingroup templatetags 
 */
function wpmobi_ordered_tag_list( $num ) {
	global $wpdb;
 ...
 *		\ingroup templatetags 
 */
function wpmobi_the_current_page_url() {
	echo wpmobi_get_current_page_url();
}
 ...
 *		\ingroup templatetags 
 */
function wpmobi_get_current_page_url() {
	return apply_filters( 'wpmobi_current_page_url', $_SERVER['REQUEST_URI'] );	
}
