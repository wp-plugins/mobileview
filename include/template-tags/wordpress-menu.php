...
function wpmobi_get_wordpress_menu_items( $parent = 0 ) {
	global $wpdb;
...
function wpmobi_wordpress_menu_has_children( $parent ) {
	global $wpdb;
...
function wpmobi_build_wordpress_menu_tree( $parent = 0, $depth = 1, &$data, $exclude_disabled = false, $parent_object = false ) {
	global $wpmobi;
	
	$items = wpmobi_get_wordpress_menu_items( $parent );
	
	if ( $depth > 1 && $parent_object ) {
		$menu_data = wpmobi_create_menu_item(
			...
			if ( $object_id ) {
				...
				switch( $menu_item_type ) {
					case 'post_type':					
						...
						$menu_class = $wpmobi->get_class_for_webapp_ignore( get_permalink( $object_id ) );
						...
						if ( wpmobi_wordpress_menu_has_children( $item->ID ) ) {
							$sub_menu = array();
							
							wpmobi_build_wordpress_menu_tree( $item->ID, $depth + 1, $sub_menu, $exclude_disabled, $menu_data );
					...
					case 'taxonomy':
						...
						$menu_data = wpmobi_create_menu_item(
							$item->ID + WPMOBI_ICON_CATEGORY_BASE,
							$depth,
							$post_title,
							'link',
							false,
							...
							false,
							$link,
							$wpmobi->get_class_for_webapp_ignore( $link )
						);				
						...
						break;
					case 'custom':
						$menu_class = $wpmobi->get_class_for_webapp_ignore( get_post_meta( $item->ID, '_menu_item_url', TRUE ) );
						...
						$menu_data = wpmobi_create_menu_item(
							$item->ID + WPMOBI_ICON_CUSTOM_BASE,
							$depth,
							$post_title,
							'link',
							false,
