...
//! The current version of the CLC API
define( 'CLC_API_VERSION', '1.2.2' );

//! The URL used to access the CLC API
define( 'CLC_API_URL', ' ' . CLC_API_VERSION );

//! The WordPress setting name for the token information 
define( 'CLC_TOKEN_INFO_SETTING', 'clcid_token' );

define( 'CLC_AUTHENTICATION_ATTEMPTS', 3 );

define( 'CLC_API_TIMEOUT', 20 );
...
class CLCAPI {
	...
	//! The access token for communincating with the CLC API
	var $token;
	
	//! The time variance between this server and the CLC API servers.  Used to generate timestamps to prevent replay attacks.
	var $time_variance;
	
	//! Indicates that the token has been checked against the server at least once
	var $token_checked;
	
	//! Indicates the UNIX time which the CLC access token will expire at
	var $token_expires;
	...
	function CLCAPI( $clcid, $license_key ) {
		$this->clcid = $clcid;
		...
		$token_info = get_option( CLC_TOKEN_INFO_SETTING, false );
		if ( $token_info ) {
	...
	/*!		\brief Used to perform a CLC API request
	 *		The CLC API is similar to REST in terms of usage, and similar to OAuth in terms of authentication.
	 ...
	 *		http://api.colorlabsproject.com/api/v/1.1/user/get_info/
	 *
	 *		\param method the name of the CLC API method to execute
	 *		\param command the name of the CLC API command to execute
	 *		\param params an array of parameters to pass to the CLC API request
	...
	function do_api_request( $method, $command, $params = array(), $do_auth = false ) {
		$url = CLC_API_URL . "/{$method}/{$command}/";
		
		// Always use the PHP serialization method for data
		$params['format'] = 'php';
		
		if ( $do_auth ) {
			// Add the timestamp into the request, offseting it by the difference between this server's time and the CLC server's time
        ...
        $options = array( 'method' => 'POST', 'timeout' => CLC_API_TIMEOUT, 'body' => $body );
        $options['headers'] = array(
            'Content-Type' => 'application/x-www-form-urlencoded; charset=' . get_option('blog_charset'),
            'Content-Length' => strlen( $body ),
            'User-Agent' => 'WordPress/' . get_bloginfo("version") . '/WPMobi',
        ...
        if ( !is_wp_error( $raw_response ) ) {
        	...
        	if ( $raw_response['response']['code'] == 200 ) {
        		...
        		WPMOBI_DEBUG( DEBUG_WARNING, "Unable to connect to server. Response code is " . $raw_response['response']['code'] );
        	}
        } 
	...
	/*!		\brief Returns information about user associated with the CLCID
	 *		This method returns information about the user associated with the CLCID.  This information includes
	...
	/*! 	\brief Authenticates the CLCID and password and obtains an access token from the server.
	 *		This method negotiates an access token using the CLCID and password passed into
	 *		this classes' constructor.  The authentication flow is as follows:
	 *
	 *			1. Client => Requests authorization nonce from server 
	 *			2. Server => Returns nonce, associating it with the CLCID supplied
	 *			3. Client => Generates encrypted authentication packet using nonce and credentials
	 *			4. Server => Verifies credentials against CLCID information stored on the server
	 ...
	 *		associated with the CLCID.
	 */		
	function authenticate() {
		WPMOBI_DEBUG( WPMOBI_ERROR, "Authenticating" );
		...
		if ( $nonce ) {
			...
			if ( $auth_result ) {
				if ( $auth_result['status'] == 'ok' ) {
					...
					update_option( CLC_TOKEN_INFO_SETTING, $token_info );
	...
	function invalidate_all_tokens() {
		delete_option( CLC_TOKEN_INFO_SETTING );	
	}
	
	/*! 	\brief Checks to see if the access token is still valid
	 *		The result of a successful authentication against the CLC API is an access token 
	...
	function check_token() {
		if ( !$this->clcid || !$this->license_key ) {
			return false;	
		}
		...
		if ( $this->token ) {
			$params = array();
			$params['clcid'] = $this->clcid;
			
			WPMOBI_DEBUG( WPMOBI_ERROR, "Performing server token check" );
	...
	function try_multiple_authentications() {
		...
		while ( true ) {
			...
			if ( $attempts > CLC_AUTHENTICATION_ATTEMPTS ) {
				break;	
			}	
		}	
	...
	function internal_check_token() {
		...
		if ( time() > ( $this->token_expires - 15 ) ) {
			WPMOBI_DEBUG( WPMOBI_INFO, "Authenticating due to expiry" );
		...
		if ( $succeeded && !$this->token_checked ) {	
			...
			if ( ( time() - $this->last_token_check ) > 300 ) {	
				WPMOBI_DEBUG( WPMOBI_INFO, 'Forcing token check due to cache expiry' );
