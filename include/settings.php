...
class WPMobiSettings extends stdClass {	};

// Settings are added as setting_name => default_value
class WPMobiDefaultSettings extends WPMobiSettings {
	function WPMobiDefaultSettings() {
		
		// default settings go here
		$this->site_title = get_bloginfo( 'name' );	
		$this->desktop_is_first_view = false;
		$this->welcome_alert = '';
		$this->fourohfour_message = sprintf( __('%sSorry, the content you were looking for cannot be found. Please use the menu in the header to access the site search, or browse other site content.%s', 'wpmobi-me'), '<p>', '</p>' );
		$this->footer_message = '';

		$this->custom_css_file = '';
		$this->current_theme_friendly_name = __( 'HipNews', 'wpmobi-me' );
		$this->current_theme_location = '/plugins/' . WPMOBI_ROOT_DIR . '/themes';
		$this->current_theme_name = 'hipnews';
		$this->current_theme_skin = 'none';
		$this->custom_menu_name = 'none';
		
		$this->show_wpmobi_in_footer = true;
		...
		$this->debug_log_level = WPMOBI_SECURITY;
		...
		// CLC API
		$this->clcid = '';
		$this->wpmobi_license_key = '';
		
		// Real menu settings
		$this->menu_icons = array();
		$this->default_menu_icon = '/plugins/' . WPMOBI_ROOT_DIR . '/resources/icons/hipnews/Default.png';
		...
		$this->last_clcid_time = 0;
		$this->last_clcid_result = false;		
		$this->clcid_had_license = false;	
		$this->last_clcid_licenses = 0;
		...
		if ( $wpmobi->plugin_hooks && count( $wpmobi->plugin_hooks ) ) {
			foreach( $wpmobi->plugin_hooks as $name => $hook ) {
				$proper_name = "plugin_disable_" . str_replace( '-', '_', $name );				
				$this->$proper_name = false;
			}
		}
		
		$this->put_wpmobi_in_appearance_menu = false;
