...
function ready () {
	...
	div.innerHTML = touchIcon + options.message.replace( '[device]', '%device' ).replace( '[icon]', '%icon' ).replace('%device', platform).replace('%icon', OSVersion >= 4.2 ? '<span class="share"></span>' : '<span class="plus">+</span>') + (options.arrow ? '<span class="arrow"></span>' : '') + '<span class="close">\u00D7</span>';
